...
class RuleEngine:
    ...
    """
    def __init__(self):
        logger.debug("Initializing RuleEngine...")
    ...
    def analyze(self, text: str):
        ...
        for category, keywords in self.dark_pattern_keywords.items():
            for keyword in keywords:
                if keyword in text_lower:
                    logger.debug(f"Rule match: Dark Pattern '{category}' triggered by keyword '{keyword}'")
                    return {
        ...
        for keyword in self.phishing_keywords:
            if keyword in text_lower:
                 logger.debug(f"Rule match: Phishing triggered by keyword '{keyword}'")
                 return {
